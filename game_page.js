...
// Function to get the name of the player who asked the current question
function getQuestionPlayerName() {
    return currentPlayerTurn === 1 ? player1Name : player2Name;
}

// Function to get the name of the player who answered the current question
function getAnswerPlayerName() {
    return currentPlayerTurn === 1 ? player2Name : player1Name;
}

// Function to update the display of question and answer turns
function updateTurnDisplay() {
    const questionTurn = currentPlayerTurn === 1 ? player1Name : player2Name;
    const answerTurn = currentPlayerTurn === 1 ? player2Name : player1Name;
    document.getElementById("question_turn").textContent = `Question Turn: ${questionTurn}`;
    document.getElementById("answer_turn").textContent = `Answer Turn: ${answerTurn}`;
}

// After initializing player names and scores, update the display for the first turn
updateTurnDisplay();
...
function askQuestion() {
    ...
    questionParagraph.textContent = `${getQuestionPlayerName()} asks: What is ${currentNumber1} * ${currentNumber2}?`;
...
function checkAnswer() {
    ...
    if (playerAnswer === correctAnswer) {
        feedbackDiv.textContent = `${getAnswerPlayerName()} got it right!`;
        // Update the score of the current player who answered correctly
        ...
        feedbackDiv.textContent = `${getAnswerPlayerName()} got it wrong! Correct answer is ${correctAnswer}.`;
    }
    
    // Switch the turn to the other player
    currentPlayerTurn = currentPlayerTurn === 1 ? 2 : 1;
    
    // Update the display for the next turn
    updateTurnDisplay();
